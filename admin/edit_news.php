...
<div class="max-w-2xl">
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <h2 class="font-bold text-gray-800 text-lg mb-5">Edit Berita</h2>
    ...
    <form method="POST" class="space-y-4">
      <div>
        ...
        <input type="text" name="judul" value="<?= htmlspecialchars($_POST['judul'] ?? $n['judul']) ?>"
      ...
      <div>
        ...
        <input type="text" name="gambar" value="<?= htmlspecialchars($_POST['gambar'] ?? $n['gambar']) ?>"
      ...
      <div>
        ...
        <textarea name="konten" rows="8"
          class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1d6ec5] resize-none" required><?= htmlspecialchars($_POST['konten'] ?? $n['konten']) ?></textarea>
      </div>
      ...
      <div>
        ...
        <div class="flex items-center gap-4">
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="radio" name="status" value="publish" <?= ($_POST['status']??$n['status'])==='publish'?'checked':'' ?>>
            <span class="text-sm text-gray-700">Publish</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="radio" name="status" value="draft" <?= ($_POST['status']??$n['status'])==='draft'?'checked':'' ?>>
